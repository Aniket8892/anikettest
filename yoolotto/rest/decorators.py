...
from yoolotto.second_chance.models import VideoProvider, BannerAdProvider,ProviderVersionStatus

from yoolotto.rest.exceptions import WebServiceException, WebServiceObjectDoesNotExist, \
    WebServiceAuthenticationFailed,UserBlockedException
...
#from yoolotto.lottery.models import VideoPriorityList1
from yoolotto.util.utilfunctions import get_or_create_country_object
class Authenticate(object):
    ...
    def __call__(self, fn):
        def wrapped(*args, **kwargs):
            request = args[1]
            print "app",request.META["HTTP_YOO_APP_VERSION"]
...
### For postman hitting changes #####
#### Modifies providers list added provider type wid provider name req for ProviderVersionStatus######
...
def check_suspicious_user(function):
    def wrap(*args, **kwargs):
        request = args[1]
        #print "req",request
        country_name = request.META.get("HTTP_YOO_COUNTRY_NAME","USA")
        country_code = request.META.get("HTTP_YOO_COUNTRY_CODE","US")
        print "check",country_name,country_code
        ...
        if not client_ip_status.is_blocked:
            ...
            if not user.is_blocked:
                ...
                print "providers_list",providers_list
                try:
                ...
                #### Aniket added country for internationsation #####
                country_obj=get_or_create_country_object(country_code,country_name)
                print "obj",country_obj
                black_listed_providers=[]
                for prov in providers_list:
                    print "provs",prov
                    for key,value in prov.iteritems():
                        print key,value
                        try:
                            print "testrs",black_listed_providers.append(ProviderVersionStatus.objects.filter(device_type=device_type,app_version=str(request.META["HTTP_YOO_APP_VERSION"]),country=country_obj,provider_name=value,provider_type=key,isEnable=False).values('provider_name')[0]["provider_name"])
                        except:
                            pass
                            #print "testrs failed"
                   
                print "blocked",black_listed_providers
                #####################################################
                ...
                if not device_created:
                    if not user_device_status.is_blocked:
                        ...
                        if user_last_impresssion:
                            ...
                            print "reason",reason_for_suspision 
                    else:  
...
# def check_suspicious_user_new(function):
#     def wrap(*args, **kwargs):
#         request = args[1]
#         client_ip = get_client_ip(request)
#         client_ip_status,ip_created = IPStatus.objects.get_or_create(ip_address = client_ip)
