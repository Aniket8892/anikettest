...
from yoolotto.coin.manager import CoinTicketManager, CoinTicketManager1
...
class LotteryGame(models.Model):
    ...
    def representation(self, user=None):
            ...
            if user:
                try:
                    tickets = user.tickets.filter(draw__component__parent=self,fantasy = False)
                    ...
                    tickets = user.tickets.filter(draw__component__parent=self, draw__result__isnull=False,fantasy = False)
...
class LotteryDraw(models.Model):
    ...
    class Meta:
        unique_together = ('component', 'date', 'division',)
...
class LotteryTicket(models.Model):
    ...
    add_coins = models.BooleanField(default = False)
    add_winnings = models.BooleanField(default = False)
    coins_deduction = models.BooleanField(default = False)
    updated_at= models.DateTimeField()
    ticket_history_recorded = models.BooleanField(default = False)
    ...
    def update1(self,device_type,version, multicoin,countt, device=None, Client_login=None, full=False):
        if full:
            self.calculate_winnings(device_type,version)
        if not multicoin:
            allocated = self.update_coins1(device,countt, Client_login, save=False)
        else:
            allocated = 0
        self.update_coupons(save=False)
        print "in update1"
        self.update_winnings(device_type,version,save=False)
        
        self.save()
                
        return allocated

    def update_coins1(self, device,countt, Client_login, save=False):
        manager = CoinTicketManager1(device,countt, Client_login, ticket=self)
        manager.update1()
        if save:
            self.save()
    ...
    def update_winnings(self,device_type,version,save=True):
        print "in update winnings"
        if not self.draw.result:
            self.winnings = None
            if save:
                save.save()
        ...
        if self.fantasy == False:
            ...
            for play in self.plays.all():
                if play.winnings is None:
                    continue
                total = total + float(play.winnings)
                print "total in update winnings",total
                result += 1
            ...
            if result == 0:
                ...
                self.winnings = total
            print "winningss",self.winnings        
            if save:
                self.save()
            
    def calculate_winnings(self,device_type,version):
        print "in calculate_winnings"
	print self.plays
        map(lambda x: x.update_winnings(device_type,version), self.plays.exclude(play=None))

    def representation1(self,
        representation1=representations.TicketRepresentationShortList1,
        *args, **kwargs):
        return representation1(self)(*args, **kwargs)
            
    def representation(self,email,version,device_type,
        representation=representations.TicketRepresentationShortList,
        *args, **kwargs):
        return representation(self,email,version,device_type)(*args, **kwargs)
...
class LotteryTicketPlay(models.Model):
    ...
    def calculate_megamillion_winnings(self, winnings, draw, game):
        print "in calculate_megamillion_winnings"
        """
        This will return the winning amount as per request
        """
    ...
    def calculate_fantasymegamillion_winnings_iphone(self,_winnings):
        play_result = _winnings
	winnings = "0@"
        if play_result == "MEGABALL_ONLY":
            winnings = "0@"
    ...
    def calculate_fantasypowerball_winnings_iphone(self,_winnings):
        play_result = _winnings
        winnings = "0@"
        if play_result == "POWERBALL_ONLY":
            winnings = "0@"
    ...
    def update_winnings(self,device_type,version,save=True):
        print "innnnnnnnnnnnnnnnnnnnnnn update winningssss"
        from yoolotto.lottery.game.manager import GameManager
        ...
        if self.ticket.fantasy == False:    
            if self.ticket.draw.result:
                ...
                print "drawwwwwwwwwwwwwww",self.ticket.draw.result
                print self.ticket.draw.id
                print winnings
                try:                                                                                            
            ...
            if self.ticket.draw.result:
                ...
                if not isinstance(_winnings, list):
                    ...
                    winnings = _winnings[0]
                try:
                    print "in tryyyyyyyyyyyyyyyyyyy"
                    if device_type == "IPHONE" and version in ["8.7","8.6"]:
                        print "righttttttttttttttttttt hereeeeeeeeeeeee"
                        if int(game_type) == 0 or int(game_type) == 13:
                            winnings_cal = self.calculate_fantasymegamillion_winnings_iphone(_winnings)
                        elif int(game_type) == 1 or int(game_type) == 11:
                            winnings_cal = self.calculate_fantasypowerball_winnings_iphone(_winnings)
                    else:
                        if int(game_type) == 0 or int(game_type) == 13:
...
class Yoocoins_Info(models.Model):
    data = models.TextField(max_length=200,null=True)
    yoocoins = models.TextField(max_length=200,null=True)
    coins_info = models.TextField(max_length=200,null=True)
    valid_image = models.TextField(max_length=200,blank=True, null=True)
    hot_icon_number = models.IntegerField(default=0)

    def representation(self):
        result = {
            "id": self.pk,
            "data": self.data,
            "yoocoins": self.yoocoins,
            "coins_info":self.coins_info,
            "valid_image":self.valid_image,
            "hot_icon_number":self.hot_icon_number,
        }
        return result

    class Meta:
        db_table = u"yoocoins_info"

class Yoocoins_Info_7_0(models.Model):
    data = models.TextField(null=True)
    yoocoins = models.TextField(null=True)
    ...
    class Meta:
        db_table = u"coins_info"


class UserTestimonials(models.Model):
    name = models.CharField(max_length = 255,blank=True, null=True)
    content = models.TextField(blank=True, null=True)
    profile_pic = models.ImageField(blank=True,upload_to="static/testimonials/", default='static/testimonials/image.jpeg', null=True,max_length=300)
    ...
    def representation(self):
        result = {
            ...
            "image": self.profile_pic.url
        }
        
        return result

    class Meta:
        db_table = u"user_testimonials"

class RulesAndInfo(models.Model):
    ...
    class Meta:
        db_table = u"rules_and_info"

class FantasyWinnerInfo(models.Model):
    user_email = models.CharField(max_length = 255)
    login_email = models.CharField(max_length = 255)
    ticket_id = models.CharField(max_length = 255)
    winning_amount = models.FloatField(default = 0)

    class Meta:
        db_table = u"fantasy_winner_info"

class FantasyLotteryWinningsHistory(models.Model):
    ticket = models.ForeignKey(LotteryTicket, related_name="fantasy_ticket")
    #play = models.ForeignKey(LotteryTicketPlay, related_name="fantasy_ticket_play")
    user = models.ForeignKey("user.YooLottoUser",related_name="fantasy_tickets")
    winnings = models.FloatField(default = 0)
    winnings_coins = models.FloatField(default = 0)
    added_at = models.DateTimeField(auto_now_add=True)
    ...
    class Meta:
        db_table = u"fantasy_ticket_winnings_history"

class VideoPriorityList(models.Model):
    priority_list_id = models.IntegerField(default=0)
    video_provider = models.CharField(max_length = 255,blank=True, null=True)
    ...
    def representation(self):
        result = {
            ...
            "value": self.video_provider
        }
        
        return result

    class Meta:
        db_table = u"video_priority_list"

class VideoPriorityList1(models.Model):
    priority_list_id = models.IntegerField(default=0)
    video_provider = models.CharField(max_length = 255,blank=True, null=True)
    device_type = models.CharField(max_length = 100,blank=True, null=True)
    app_version = models.CharField(max_length = 100,blank=True, null=True)
    ...
    class Meta:
        db_table = u"video_priority_list1"

class AeservPLCPriorityList(models.Model):
    priority_list_id = models.IntegerField(default=0)
    plc_provider = models.CharField(max_length = 255,blank=True, null=True)
    plc_value = models.CharField(max_length = 255,blank=True, null=True)
    device_type = models.CharField(max_length = 100,blank=True, null=True)
    app_version = models.CharField(max_length = 100,blank=True, null=True)
    isEnable = models.BooleanField(default=True)
    added_at = models.DateTimeField(auto_now_add=True)
    ...
    def representation(self):
        result = {
            "id": self.priority_list_id,
            "name": self.plc_provider,
            "value":self.plc_value
        }
        return result
    class Meta:
        db_table = u"aeserv_plc_list"
