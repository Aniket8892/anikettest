...
class CoinTicketManager1(object):
    """
    Responsible for determining and updating the number of YooCoins granted to 
    a user for a ticket. Enforces daily limits for coins gained from submissions
    ...
    def __init__(self, device,countt, user_email,ticket):
        self._ticket = ticket
        self._wallet = ticket.user.get_wallet()
        self._allocated = None
        self._device = device
    ...
    def add_coins_on_submission(self):
        print "in add_coins_on submissi"
	from yoolotto.lottery.models import LotteryTicketAvailable
        ...
        print "hey the email is .......", self._user_email
        print "result123 is ", self._user_email is not None
        print "result345 is ", self._user_email != ""
        ...
        print "countttttttttttt",count1
        if count1 > self._countt:
